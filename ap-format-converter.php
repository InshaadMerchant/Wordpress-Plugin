...
Return ONLY the converted article content in proper HTML format with paragraphs.";
    ...
    // Bulk convert all posts to AP format (admin action)
    public function bulk_convert_to_ap() {
        if (!current_user_can('manage_options')) {
            wp_die('Unauthorized');
        }
        
        check_admin_referer('bulk_convert_to_ap');
        ...
        foreach ($posts as $post) {
            if ($this->convert_single_post_to_ap($post)) {
                $converted_count++;
            }
        }
        
        wp_redirect(admin_url('admin.php?page=format-converter&converted=' . $converted_count));
        exit;
    }
    
    // Bulk revert all posts to original format (admin action)
    public function bulk_revert_to_original() {
        if (!current_user_can('manage_options')) {
            wp_die('Unauthorized');
        }
        
        check_admin_referer('bulk_revert_to_original');
        
        $reverted_count = $this->bulk_revert_all_posts();
        
        wp_redirect(admin_url('admin.php?page=format-converter&reverted=' . $reverted_count));
        exit;
    }
}
